...
export default function agentModeExtension(pi: ExtensionAPI) {
	...
	let _agentFooterRequestRender: (() => void) | null = null;

	function _updateAgentFooter(ctx: ExtensionContext): void {
		if (!_agentFooterRequestRender) {
			// First call — install the custom footer
			ctx.ui.setFooter((tui, theme, footerData) => {
				_agentFooterRequestRender = () => tui.requestRender();
				const unsub = footerData.onBranchChange(() => tui.requestRender());
				return {
					dispose: () => {
						unsub();
						_agentFooterRequestRender = null;
					},
					invalidate() {},
					render(width: number): string[] {
						const branch = footerData.getGitBranch();
						const statuses = footerData.getExtensionStatuses();
						const agentText = statuses.get("agent");
						const cwd = ctx.cwd;
						const home = process.env.HOME || "";
						const displayPath = cwd.startsWith(home) ? "~" + cwd.slice(home.length) : cwd;

						// Line 1: git repo left, agent status right
						const left1 = theme.fg("dim", displayPath + (branch ? " (" + branch + ")" : ""));
						const right1 = agentText ? theme.fg("dim", agentText) : "";

						const leftWidth = visibleWidth(left1);
						const rightWidth = right1 ? visibleWidth(right1) : 0;
						const pad = right1 ? " ".repeat(Math.max(1, width - leftWidth - rightWidth)) : "";
						const line1 = truncateToWidth(left1 + pad + (right1 || ""), width);

						// Line 2: token/cost stats
						...
						const line2 = truncateToWidth(left2 + " ".repeat(Math.max(1, width - left2w - right2w)) + right2, width);
			...
			// Footer already installed — trigger re-render so it picks up new status
			_agentFooterRequestRender();
		}
	}
